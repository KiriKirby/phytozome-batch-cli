...
The MSA coordinate display option is recorded as part of the shared tree/MSA payload metadata. When `Show PHgo row coordinates` is enabled, MSA stores the coordinate prefix separately from the true Jalview sequence name. Snapshots must therefore preserve both the raw/base display name and the optional coordinate prefix so reopening a snapshot keeps Jalview rename/search/menu behavior based on the raw display name while the left sequence list can still draw the `[canvas,row]` prefix.
