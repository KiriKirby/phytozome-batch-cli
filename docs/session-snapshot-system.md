...
- `modules/keyword-source-state-v4.xml` stores keyword source-engine metadata, including NCBI Entrez database, record type, engine schema, search-type/result-domain context, linked-fallback provenance, and replacement/update source state.
...
For the current `v2.7` implementation, artifact packing is intentionally narrow:
...
Canvas MSA snapshots are stored in a separate `canvas-msa-state-v1` module. The module preserves the row state (`green`, `yellow`, and `red`), the last shared tree/MSA payload and aligned FASTA, and Jalview-owned durable state such as groups, sequence features, annotations, markers, alignment settings, per-sequence descriptions, per-sequence display-name edits, and per-sequence text edits when the bridge provides them. The Jalview bridge saves that durable state through `PUT /sessions/<id>/msa/state` during startup, selection changes, File > Save, and Apply, and PHgo serves it back through `GET /sessions/<id>/msa/state` for snapshot collection. File > Save collects sequence features using the same Jalview model that project save uses (`type`, `description`, `begin`, `end`, `score`, `feature_group`, links, and `other_details`) and only updates the MSA snapshot state; File > Apply also synchronizes allowed edits back to Canvas, where sequence-name edits become locked `display_name` values and sequence edits update Canvas sequence data after uppercasing letters. PHgo maintains one selected-row set for both tree and MSA: green rows are sent to both, yellow rows are unchecked for both but marked yellow in PHgo because they were unchecked from MSA Apply, and red rows are ordinary unchecked rows.

PHgo row coordinate prefixes are recorded as part of the shared tree/MSA payload metadata. MSA always stores and draws the coordinate prefix separately from the true Jalview sequence name. Tree coordinate labels are controlled only by Reactree viewer state through the View > Labels > `Coord` toggle, which defaults off. Snapshots must therefore preserve both the raw/base display name and the coordinate prefix so reopening a snapshot keeps Jalview rename/search/menu behavior based on the raw display name while the left sequence list can still draw the `[canvas,row]` prefix.
