...

{
  ...
  public
    ...
    property ErrorMessage: AnsiString read ErrMessage;
...
{$IFDEF VISUAL_BUILD}
  result := FShowProgress and (FProgressDlg <> nil);
{$ENDIF}
end;

procedure TClustalWThread.SetShowProgress(value: boolean);
begin
  FShowProgress := value;
end;
...
procedure TClustalWThread.SetSeqArray(seqs : TStringList);
   ...
   procedure p_encode(seq: PAnsiChar; l: integer; seqIndex: integer);
   var
     i, code: integer;
   begin
...
//*	static char *aacids="CSTPAGNDEQHRKMILVFYW";*/
      ...
      for i := 1 to l do
         ...
         begin
           code := res_index(amino_acid_codes, seq[i]);
           if code < 0 then
             raise Exception.Create('Unsupported protein residue "' + String(seq[i]) + '" at sequence ' + IntToStr(seqIndex) + ', site ' + IntToStr(i));
	         seq[i] := AnsiChar(Chr(code));
         end;
      seq[l+1] := AnsiChar(-3);
   end;

   procedure n_encode(seq: PAnsiChar; l: integer; seqIndex: integer);
   var
     i, code: integer;
   begin
      ...
      for i := 1 to l do
	    ...
	    seq[i] := AnsiChar(Chr(gap_pos2))     //* this is the code for a gap in */
         ...
         begin
           code := res_index(amino_acid_codes, seq[i]);
           if code < 0 then
             raise Exception.Create('Unsupported nucleotide residue "' + String(seq[i]) + '" at sequence ' + IntToStr(seqIndex) + ', site ' + IntToStr(i));
	         seq[i] := AnsiChar(Chr(code));
         end;
      seq[l+1] := AnsiChar(-3);
   end;
...
begin
  ...
  //  if not dnaflag then
    ...
    for i := 0 to nseqs-1 do
      if dnaflag then
        n_encode(seq_array[i+1], Length(seqs[i]), i + 1)  //* encode the sequence*/
      else				     //* as ints  */
        p_encode(seq_array[i+1], Length(seqs[i]), i + 1);
...
begin
 ...
 //            info('Sequences (%d:%d) Aligned. Score:  %d', (pint)si+1,(pint)sj+1, (pint)mm_score);
  ...
  if not Terminated then
    ...
    begin
       {$IFNDEF VISUAL_BUILD}
       error_nv('error in TClustalWThread.pairalign at sequence pair (' + IntToStr(seq1) + ',' + IntToStr(seq2) +
         '), n=' + IntToStr(n) + ', m=' + IntToStr(m) + ', se1=' + IntToStr(se1) +
         ', se2=' + IntToStr(se2) + ', sb1=' + IntToStr(sb1) + ', sb2=' + IntToStr(sb2) +
         ': ' + E.Message);
       {$ENDIF}
    end;
  end;
