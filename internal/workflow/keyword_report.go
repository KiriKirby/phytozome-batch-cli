...
func speciesSourceNotes(species model.SpeciesCandidate, sourceName string) string {
	...
	case "lemna":
		return "lemna.org keyword rows are generated by resolving the selected release from the download directory, opening the release GFF3 file, scanning searchable GFF3 features, and enriching matched rows with already-loaded AHRD records when available. Download source addresses are stored in fields such as lemna_gff_url; row-level gene_report_url is only populated when the workflow received a real source page URL, such as a user-provided report URL."
	case "phytozome":
		return "Phytozome keyword rows are generated through the normal source client path. Report URLs are parsed as /report/gene, /report/transcript, or /report/protein; specific identifiers are tried through gene, transcript, and protein lookup paths; otherwise the Elasticsearch-backed Phytozome keyword endpoint is used with the selected proteome ID. Row-level gene_report_url is only populated when the workflow received a real source page URL, such as a user-provided report URL."
	default:
...
func keywordQualityChecks(groups []model.KeywordSearchGroup, rows []model.KeywordResultRow, allRows []model.KeywordResultRow, settings exportSettings, sequenceAudit report.SequenceAudit) []report.QualityCheck {
	...
	checks := []report.QualityCheck{
		...
		keywordSourceURLAuthenticityCheck(tableRows, totalRows),
...
func keywordSourceURLAuthenticityCheck(rows []model.KeywordResultRow, totalRows int) report.QualityCheck {
	withURL := countKeywordRowsWhere(rows, func(row model.KeywordResultRow) bool {
		return strings.TrimSpace(row.GeneReportURL) != ""
	})
	return report.QualityCheck{
		Name:        "Row-level report URL authenticity",
		Result:      "pass",
		Count:       fmt.Sprintf("%d with URL / %d blank", withURL, totalRows-withURL),
		Rule:        "blank is correct unless the workflow obtained a real source page URL",
		Explanation: "gene_report_url is reserved for authentic source webpage URLs. Rows from source files, APIs, BLAST hits, or local indexes leave this field empty instead of synthesizing route-shaped links.",
		Source:      "generated keyword table",
	}
}
...
func keywordColumnLineageForHeader(header string, rows []model.KeywordResultRow) report.ColumnLineage {
	...
	case "search_type":
		return report.ColumnLineage{ID: header, Column: display, Meaning: firstNonEmpty(english, "Search program selected by the keyword search engine."), EnglishDetail: english, ChineseDetail: chinese, JapaneseDetail: japanese, Source: "new search engine", CollectionMethod: "classified before searching; sources with automatic wide-search fallback record that fallback explicitly, while strict TAIR keyword search does not auto-fallback", BlankMeaning: "legacy row or source did not report search type", UsedInStats: "yes"}
	case "label_name":
