			...
			try {
				...
				if (isApp && applet.__Info.headless) {
					try {
						...
						window.__PHGOMainRunError = { main: clazz, args: applet.__Info.args || [], message: String(e && e.message || e), stack: e && e.stack || "", className: e && e.__CLASS_NAME__ || "", javaClass: e && e.getClass$ && e.getClass$().__CLASS_NAME__ || "" };
						throw e;
					}
				} else {
...
Clazz.loadScript = function(file) {
  ...
  try{
    ...
    if (file.indexOf("/j2s/core/") >= 0) {
      window.__PHGOLastSwingJSError = { file: file, message: String(e && e.message || e), stack: e && e.stack || "", lastEvalError: Clazz._lastEvalError || "" };
...
CLPM.showStatus = function (msg, fading) {
  if (msg != null && !/(error|warning|warn|failed|failure|exception|unable|cannot|denied)/i.test("" + msg)) {
    if (monitorEl) CLPM.hideMonitor();
    return;
  }
  if (!monitorEl) {
    createHandle ();
