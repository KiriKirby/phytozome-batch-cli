...
var pixelRatio=canvas.__j2sPixelRatio || 1;
if (pixelRatio != 1) {
var readCanvas=$I$(10).createElement("canvas", null);
readCanvas.width=w;
readCanvas.height=h;
var readCtx=readCanvas.getContext("2d");
readCtx.drawImage(canvas, 0, 0, canvas.width, canvas.height, 0, 0, w, h);
data=readCtx.getImageData(0, 0, w, h).data;
} else {
...
var pixelRatio=Math.max(1, (self && self.devicePixelRatio) || (globalThis && globalThis.devicePixelRatio) || 1);
canvas.__j2sLogicalWidth=w;
canvas.__j2sLogicalHeight=h;
canvas.__j2sPixelRatio=pixelRatio;
canvas.width = Math.max(0, Math.round(w * pixelRatio));
canvas.height = Math.max(0, Math.round(h * pixelRatio));
