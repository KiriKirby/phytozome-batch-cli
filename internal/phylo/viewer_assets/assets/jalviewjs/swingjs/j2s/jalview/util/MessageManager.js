...
C$.rb=$I$(5).getBundle$S$java_util_Locale$java_util_ResourceBundle_Control("lang.Messages", $I$(3).ROOT, control);
} catch (e$$) {
if (Clazz.exceptionOf(e$$,"Exception")){
var q = e$$;
{
C$.log.warn$O("Exception when initting Locale for i18n messages\n" + (q.getMessage$ ? q.getMessage$() : q.message || q.toString()));
if (q.printStackTrace$) q.printStackTrace$();
 else if (q.stack) C$.log.warn$O(q.stack);
}
} else if (Clazz.exceptionOf(e$$,"Error")){
var v = e$$;
{
C$.log.warn$O("Error when initting Locale for i18n messages\n" + (v.getMessage$ ? v.getMessage$() : v.message || v.toString()));
if (v.printStackTrace$) v.printStackTrace$();
 else if (v.stack) C$.log.warn$O(v.stack);
}
} else {
throw e$$;
}
