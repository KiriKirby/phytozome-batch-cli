...
var seq=null;
id=id.trim$();
if (id.indexOf$S("phgo_name64:") == 0) {
var firstSpace=id.indexOf$S(" ");
var token=firstSpace > -1 ? id.substring$I$I(0, firstSpace) : id;
var encoded=token.substring$I("phgo_name64:".length);
var base64=(""+encoded).replace(/-/g, "+").replace(/_/g, "/");
base64 += "===".substring((base64.length + 3) % 4);
var binary=atob(base64);
var bytes=new Uint8Array(binary.length);
for (var i=0; i < binary.length; i++) {
bytes[i]=binary.charCodeAt(i);
}
var decoded=(typeof TextDecoder != "undefined") ? (new TextDecoder("utf-8")).decode(bytes) : decodeURIComponent(escape(binary));
seq=Clazz.new_($I$(7).c$$S$S,["PHgo", ""]);
seq.name=decoded;
seq.phgoOriginalHeader=id;
var taxonMarker="phgo_taxon_id64:";
var taxonIndex=id.indexOf$S(taxonMarker);
if (taxonIndex > -1) {
var taxonEnd=id.indexOf$S$I(" ", taxonIndex);
var taxonEncoded=taxonEnd > -1 ? id.substring$I$I(taxonIndex + taxonMarker.length, taxonEnd) : id.substring$I(taxonIndex + taxonMarker.length);
var taxonBase64=(""+taxonEncoded).replace(/-/g, "+").replace(/_/g, "/");
taxonBase64 += "===".substring((taxonBase64.length + 3) % 4);
var taxonBinary=atob(taxonBase64);
var taxonBytes=new Uint8Array(taxonBinary.length);
for (var ti=0; ti < taxonBinary.length; ti++) {
taxonBytes[ti]=taxonBinary.charCodeAt(ti);
}
seq.phgoTaxonID=(typeof TextDecoder != "undefined") ? (new TextDecoder("utf-8")).decode(taxonBytes) : decodeURIComponent(escape(taxonBinary));
}
return seq;
}
