...
var x=this.c.getX$();
var y=this.c.getY$();
if (this.isWindow && !(this.c && (this.c.__phgoMainAlignmentFrame || this.c.__phgoRootDesktopFrame)) ) {
{
var vw = Math.max(320, (window && (window.innerWidth || document.documentElement.clientWidth)) || 0);
var vh = Math.max(240, (window && (window.innerHeight || document.documentElement.clientHeight)) || 0);
var ww = this.c.getWidth && this.c.getWidth$ ? this.c.getWidth$() : (this.width || 0);
var wh = this.c.getHeight && this.c.getHeight$ ? this.c.getHeight$() : (this.height || 0);
if (ww > 0 && ww < vw) x = Math.min(Math.max(8, x), Math.max(8, vw - ww - 8));
if (wh > 0 && wh < vh) y = Math.min(Math.max(8, y), Math.max(8, vh - wh - 8));
}
}
$I$(4).setStyles(this.outerNode, ["left", (this.x=x) + "px", "top", (this.y=y) + "px"]);
}}, p$1);
