...
// local methods here for help with debugging
 ...
 var phgoMenuComponentFromNode = function(me, node) {
	 if (!node) return null;
	 var component = node["data-component"] || (node["data-ui"] && node["data-ui"].jc) || null;
	 if (component) return component;
	 var id = node.id || "";
	 if (!id) return null;
	 var popup = me && me.options && me.options.jPopupMenu;
	 var root = popup && (popup.getInvoker$ ? popup.getInvoker$() : popup.invoker);
	 var roots = [];
	 popup && roots.push(popup);
	 root && roots.push(root);
	 var seen = [];
	 var walk = function(item) {
		 if (!item || seen.indexOf(item) >= 0) return null;
		 seen.push(item);
		 var ui = item.ui || (item.getUI$ && item.getUI$()) || null;
		 if (ui && (ui.id === id || id.indexOf(ui.id + "_") === 0)) return item;
		 if (item.getMenuComponents$) {
			 var children = item.getMenuComponents$();
			 for (var i = 0; i < children.length; i++) {
				 var found = walk(children[i]);
				 if (found) return found;
			 }
		 }
		 if (item.getSubElements$) {
			 var sub = item.getSubElements$();
			 for (var j = 0; j < sub.length; j++) {
				 var foundSub = walk(sub[j]);
		 ...
		 if (item.getComponents$) {
			 ...
			 for (var k = 0; k < components.length; k++) {
				 var foundComponent = walk(components[k]);
...
// function(){me._closeSubmenus()},me.delay);
	 ...
	 case "blur":
		 ...
		 break;
	 case "select":
		 var phgoClickedItem=e(t.target).closest(".ui-j2smenu-item");
		 if (phgoClickedItem.length) {
			 me.active=phgoClickedItem;
		 } else {
			 me.active=me.active||phgoClickedItem;
		 }
		 if (!me.active.has(".ui-j2smenu").length) {
			 me.collapseAll(t,!0);
		 }
...
// if (menu.uiClassID)
    var v = menu.$ulTop.find("[role=menuitem],.ui-j2smenu-item");
...
var setMouseMenuItem = function(menu, node) {
    ...
    node._menu = menu;
    if (!node["data-component"] && node.id) {
      var instance = Swing.getInstance(menu);
      var component = phgoMenuComponentFromNode(instance, node);
      if (component) node["data-component"] = component;
    }
