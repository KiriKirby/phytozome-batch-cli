...
{
var phgoPixelRatio=canvas.__j2sPixelRatio || C$.pixelRatio || 1;
if (phgoPixelRatio != 1 ) this.ctx.setTransform(phgoPixelRatio, 0, 0, phgoPixelRatio, 0, 0);
}
this.transform=Clazz.new_($I$(3));
this.setStroke$java_awt_Stroke(Clazz.new_($I$(4)));
{
this.gc = SwingJS; this.width = canvas.__j2sLogicalWidth || canvas.width; this.height = canvas.__j2sLogicalHeight || canvas.height;
}
...
var imgNode=p$1.getImageNode$java_awt_Image.apply(this, [img]);
if (imgNode != null ) {
var srcRatio=imgNode.__j2sPixelRatio || 1;
this.ctx.drawImage(imgNode, sx1 * srcRatio, sy1 * srcRatio, (sx2 - sx1) * srcRatio, (sy2 - sy1) * srcRatio, dx1, dy1, dx2 - dx1, dy2 - dy1);
}
if (observer != null ) p$1.observe$java_awt_Image$java_awt_image_ImageObserver$Z.apply(this, [img, observer, imgNode != null ]);
}return true;
});
...
var pixelRatio=this.canvas.__j2sPixelRatio || 1;
var dataX=Math.round(x * pixelRatio);
var dataY=Math.round(y * pixelRatio);
var dataWidth=Math.max(1, Math.round(width * pixelRatio));
var dataHeight=Math.max(1, Math.round(height * pixelRatio));
if (this.buf8 == null  || x != this.lastx  || y != this.lasty  || this.nx != width  || this.ny != height ) {
this.imageData=this.ctx.getImageData(dataX, dataY, dataWidth, dataHeight);
...
var logicalCanvas=document.createElement("canvas");
logicalCanvas.width=width;
logicalCanvas.height=height;
var logicalCtx=logicalCanvas.getContext("2d");
var logicalData=logicalCtx.createImageData(width, height);
var logicalBuf=logicalData.data;
for (var pt=0, i=0, n=Math.min(width * height, pixels.length); i < n; i++) {
var argb=pixels[i];
logicalBuf[pt++]=(argb >> 16) & 255;
logicalBuf[pt++]=(argb >> 8) & 255;
logicalBuf[pt++]=argb & 255;
logicalBuf[pt++]=(isRGB ? 255 : (argb >> 24) & 255);
}
logicalCtx.putImageData(logicalData, 0, 0);
this.ctx.drawImage(logicalCanvas, x, y, width, height);
return;
...
var pixelRatio=this.canvas.__j2sPixelRatio || 1;
if (pixelRatio != 1) {
this.ctx.putImageData(this.ctx.getImageData(Math.round(x * pixelRatio), Math.round(y * pixelRatio), Math.round(width * pixelRatio), Math.round(height * pixelRatio)), Math.round((x + dx) * pixelRatio), Math.round((y + dy) * pixelRatio));
return;
}
...
{
var phgoPixelRatio = this.canvas.__j2sPixelRatio || 1;
this.ctx.setTransform (t.m00 * phgoPixelRatio, t.m10 * phgoPixelRatio, t.m01 * phgoPixelRatio, t.m11 * phgoPixelRatio, t.m02 * phgoPixelRatio, t.m12 * phgoPixelRatio);
}
