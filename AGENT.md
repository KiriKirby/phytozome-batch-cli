...
- Symbol name identification is a standalone system under `internal/labelname`; keep the package/import name for compatibility, but user-facing text should say `symbol name`. Workflows send known structured fields from source rows, BLAST rows, query sources, and FASTA/phgo headers as `AliasRankRequest` data. The symbol name system owns matching and ranking.
  ...
  - Prebuilt database workflow changes must be validated in stages before a full GitHub run: first run the local `cmd/build-symbolname-db -sample` small-sample build, then dispatch the GitHub workflow in `sample` mode, and only after that succeeds dispatch or wait for the `full` build. Do not use the full GitHub build as the first test because it can take hours.
