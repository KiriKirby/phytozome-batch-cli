...
- Symbol name identification is a standalone system under `internal/labelname`; keep the package/import name for compatibility, but user-facing text should say `symbol name`. Workflows send known structured fields from source rows, BLAST rows, query sources, and FASTA/phgo headers as `AliasRankRequest` data. The symbol name system owns matching and ranking.
  - The primary dictionary is the full NCBI Gene `gene_info.gz` from `https://ftp.ncbi.nlm.nih.gov/gene/DATA/`, covering all taxa. The app converts it into the application-local `symbolname.pgd` database. The `.pgd` metadata must record the source URL, NCBI `Last-Modified`, download time, content length, schema version, record count, and checksum.
  ...
  - Every `gene_info.gz` download/build path, including startup helper and in-app TUI installation, must show visible progress with downloaded bytes, total size when known, download speed, and database build progress/record count. The NCBI `gene_info.gz` network transfer must stay single-stream; do not use ranged multi-connection downloading against the FTP host.
...
- The bundled startup helper is also the release self-update entrypoint for packaged builds:
  ...
  - after the release self-update check, the startup helper `phgohelper.bin` checks the NCBI Gene symbol name library. If `symbolname.pgd` is missing, it shows the source, remote size, Last-Modified, and target path, then asks y/n whether to download/build now; declining skips only startup preflight, and the TUI must ask/download when symbol names are later used. If the remote NCBI `Last-Modified` is newer than the stored value, it shows the expired day count and remote size before asking whether to update. If an application update was accepted and staged, skip the symbol library check until the next launch. NCBI `gene_info.gz` download uses one official HTTP GET stream rather than ranged multi-connection downloading; build/index work may still use local concurrency where safe. Startup downloads and update downloads must show progress, speed, and build status.
