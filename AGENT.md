...
- Opening a Canvas snapshot must rebuild enough in-memory tree plan state from the saved payload, run manifest, and restored artifact files for the next tree-panel open, snapshot save, or preview-only metadata sync to continue without recomputing. If `last_payload` is empty but `viewer.payload.json` exists in artifacts, use the artifact payload as a recovery fallback. However, the first user-triggered `Refresh tree` after opening a Canvas snapshot is always a full compute refresh through `mega-phgo-runtime`; it must not reuse snapshot-restored alignment/tree artifacts even when fingerprints match.
  ...
  - PHgo-specific JalviewJS bootstrap glue belongs in the vendored `assets/jalviewjs/phgo-bridge.js` bridge layer, not in a custom MSA renderer. The bridge owns PHgo state parsing, Jalview `Info.args` construction, and post-startup SwingJS DOM adaptation. Parser behavior, display-name/taxon-id handling, DPI behavior, core SwingJS/Jalview rendering changes, and PHgo MSA row selection UI must be patched directly in the vendored JalviewJS/SwingJS source and runtime core files. For the MSA row selection column, draw and handle clicks in Jalview's native `IdCanvas`/`IdPanel` and add File > Apply through Jalview menu code; do not overlay DOM checkboxes/buttons on top of Jalview. Because `/sessions/<session>/msa` is the visible bootstrap route, root-relative PHgo paths such as `/sessions/<id>/aligned.fasta` should be passed to Jalview as same-origin absolute URLs so Jalview's resolver reaches the viewer server root.
  ...
  - The visible MSA page should present the opened Jalview alignment window as the whole page. Implement this in vendored JalviewJS/SwingJS itself: Jalview `Desktop.addInternalFrame` marks the first opened `AlignFrame` as the PHgo main alignment frame, and SwingJS `JSFrameUI` embeds and undecorates only that marked frame into `jalview-alignment-div`. Keep the Jalview root `Desktop` JFrame as an invisible full-page window-manager layer so search/dialog child windows can still render above the alignment; do not hide the whole desktop container, globally stretch `.swingjs-window`, or strip title bars/resizers from child windows through the PHgo bridge. Do not hide or replace Jalview's actual alignment content.
  - The PHgo JalviewJS MSA view is read-only with exactly one parent alignment window: the opened PHgo alignment. Do not expose file-open/add-sequence/paste/delete/remove/translate/fetch actions, and do not allow Jalview actions to create additional sibling alignment/split/input parent windows. Output-to-textbox is allowed only as a read-only utility child window for copying/export inspection, never as an input/new-alignment path. Do not let double-clicking sequence IDs launch the default EMBL-EBI search. Search/find, manual grouping/annotation actions, output textboxes, and other small utility dialogs may still open above the single alignment parent when they do not mutate PHgo-provided FASTA/alignment content.
...
- Symbol name identification is a standalone system under `internal/labelname`; keep the package/import name for compatibility, but user-facing text should say `symbol name`. Workflows send known structured fields from source rows, BLAST rows, query sources, and FASTA/phgo headers as `AliasRankRequest` data. The symbol name system owns matching and ranking.
  ...
  - TAIR keyword auto symbol name must follow the same two-phase rule as Lemna and BLAST symbol flows: pre-collect and de-duplicate lookup terms across all rows/groups, resolve them through shared cache/singleflight paths, then batch-rank through the symbol name system. Do not fall back to per-row serial remote lookups.
  - BLAST-hit auto symbol name should cache hit-level identification decisions by a stable hit/source-symbol key across repeated runs so reopening/re-filtering/export chains do not redo the same local ranking or fallback work.
...
- Performance and concurrency:
  ...
  - reuse BLAST-hit symbol name identification results by stable hit/source keys within a run so duplicate HSPs, duplicate target rows, and family-merged tables do not repeat local symbol name ranking or fallback work
