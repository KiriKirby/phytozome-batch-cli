...
- Treat the custom `mega-phgo-runtime` built from the MEGA 12.1 source tree as the only in-product computation engine for tree work:
  ...
  - Runtime freshness is defined by the files bundled in the current Windows release package. Do not add or depend on a separate runtime marker/version file inside `mega-phgo-runtime`; a folder is acceptable only when the PHgo probe and runtime-owned MUSCLE check pass.
...
- Release packaging rules:
  ...
  - Windows release packaging must copy `assets/mega-phgo-runtime/windows-amd64/runtime/` into the bundle root as `mega-phgo-runtime/`, so the shipped app works without any runtime download step.
