...
- Symbol name identification is a standalone system under `internal/labelname`; keep the package/import name for compatibility, but user-facing text should say `symbol name`. Workflows send known structured fields from source rows, BLAST rows, query sources, and FASTA/phgo headers as `AliasRankRequest` data. The symbol name system owns matching and ranking.
  ...
  - Runtime install/update checks should prefer a GitHub-hosted prebuilt symbol-name database manifest and archive first, not direct NCBI download/build. The prebuilt branch stores only `symbolname/manifest.json` and small documentation files; database archive files are distributed through GitHub Release assets and referenced by `browser_download_url` values in the manifest. Runtime downloads those assets, expands them locally to `symbolname.pgd`, and uses the manifest's NCBI split-source `Last-Modified` metadata for staleness checks. Only when the GitHub manifest/archive path is unavailable or invalid should the app fall back to direct NCBI `GENE_INFO/` split-source download and local build.
  - The prebuilt symbol-name database branch is maintained by GitHub Actions on a fixed monthly schedule plus manual dispatch. Current policy is one scheduled rebuild each month on day 18 at 02:18 UTC, with the workflow force-publishing the latest manifest branch snapshot rather than appending normal source-history commits to `master`.
  - Prebuilt database workflow changes must be validated in stages before a full GitHub run: first run local unit tests plus the local `cmd/build-symbolname-db -sample` small-sample build, then run a local `-simulate-archive-size` large-archive split/publish dry-run that does not build the database, then dispatch the GitHub workflow in `sample` mode, then dispatch GitHub `simulate` mode to exercise large multipart publishing without NCBI/database build cost, and only after all of those succeed dispatch or wait for the `full` build. Do not use the full GitHub build as the first test because it can take hours and consumes limited Actions quota.
  - GitHub publishing for the prebuilt symbol-name database must follow GitHub's documented large-file guidance: normal git branches hold only small manifest/docs files, while large binary archive parts live in Release assets. Release asset parts must stay below the documented per-asset limit with a safety margin; the full workflow currently uses 1 GiB parts. The publish tool must fail during validation when any branch file exceeds the configured Git blob limit or any Release asset exceeds the configured asset limit, before attempting network uploads. The manifest branch is published only after all referenced Release assets have uploaded successfully, and old matching assets are cleaned up only after the branch update succeeds.
  - Any prebuilt database split/archive change must test the reverse path: manifest-ordered part download, reassembly into the original compressed stream, decompression, sha256 validation against the decompressed `.pgd`, `.pgd` inspection, and one real symbol-name ranking query. Keep a gated live test for the GitHub `symbolname-db-sample` branch so real raw.githubusercontent.com multipart downloads can be verified without running the full database build.
