...
- The bundled startup cleaner is also the release self-update entrypoint for packaged builds:
  ...
  - the detached updater helper owns waiting for the cleaner PID to exit, replacing the packaged files in place, and relaunching via the normal launcher with an environment flag that skips a second cache-clean/update preflight on that relaunch
...
- Canvas tables use a fixed display schema: row number, source type, head, and fields carried by the current `phgo://species/label_name/gene_id\source_label_name/source_gene_id\row` header contract. Do not expand canvas tables with unrelated source-specific columns.
- Add canvas accepts either FASTA input or one `.pgo` snapshot reference. The FASTA path follows normal FASTA input behavior, including pasted/dragged file paths and phgo header metadata extraction only. Snapshot references follow the session-open resolver, including output file names with or without extension; a multi-table snapshot must prompt with a default-selected multi-select list. Add rows accepts FASTA only and appends to the active canvas table with row numbers continuing from the existing maximum.
- Creating a canvas from result tables uses only the rows selected at creation time. Multi-file/multi-run sources preserve their left-sidebar table separation and omit tables with no selected rows; single-file sources create one numeric canvas title.
...
- Cache layout rule:
  ...
  - packaged desktop startup uses a dedicated cache-cleaner first tab that cleans `.cache`, then checks GitHub releases for a newer packaged bundle, and finally spawns the main wizard in a fresh tab in the same WezTerm window and exits; user-created tabs inside that same WezTerm session must bypass cache cleaning and launch the main wizard directly
