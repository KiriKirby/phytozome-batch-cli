...
- Symbol name identification is a standalone system under `internal/labelname`; keep the package/import name for compatibility, but user-facing text should say `symbol name`. Workflows send known structured fields from source rows, BLAST rows, query sources, and FASTA/phgo headers as `AliasRankRequest` data. The symbol name system owns matching and ranking.
  ...
  - Runtime install/update checks should prefer a GitHub-hosted prebuilt symbol-name database manifest and archive first, not direct NCBI download/build. The prebuilt branch stores `symbolname/manifest.json` plus a compressed `symbolname/symbolname.pgd.gz`; runtime downloads that archive, expands it locally to `symbolname.pgd`, and uses the manifest's NCBI split-source `Last-Modified` metadata for staleness checks. Only when the GitHub manifest/archive path is unavailable or invalid should the app fall back to direct NCBI `GENE_INFO/` split-source download and local build.
  - The prebuilt symbol-name database branch is maintained by GitHub Actions on a fixed monthly schedule plus manual dispatch. Current policy is one scheduled rebuild each month on day 18 at 02:18 UTC, with the workflow force-publishing the latest generated branch snapshot rather than appending normal source-history commits to `master`.
  ...
  - GitHub API publishing for the prebuilt symbol-name database must keep individual archive parts at or below 4 MiB unless a newer tested limit is proven locally and by GitHub `simulate` mode. The publish tool must fail during validation when any file exceeds the configured API blob limit, before attempting network uploads. Database publish commits should be orphan snapshots by default so old multi-GB database versions do not accumulate through branch ancestry.
