...
- Treat the custom `mega-phgo-runtime` built from the MEGA 12.1 source tree as the only in-product computation engine for tree work:
  ...
  - Runtime discovery for tree work is strict: only the application-local `mega-phgo-runtime` folder next to the running app is valid. Do not search `PATH`, do not use installed MEGA/MEGA-CC folders, and do not accept a system-managed runtime location. Expanding the Canvas tree tool panel and refreshing a tree must both verify `mega-phgo-runtime/mega-phgo-runtime(.exe)` by PHgo probe plus the runtime-owned MUSCLE binary; if either is missing or invalid, use the PHgo release asset download/extract flow to populate that exact folder from the source-controlled runtime release manifest in `internal/megaphgo/runtime-release.json`.
  - PHgo runtime release packages are currently published only for Windows amd64. Linux and macOS must report that the managed PHgo runtime package is not supported until matching runtime builds are intentionally added.
  - Runtime freshness is defined only by `internal/megaphgo/runtime-release.json`: its release tag, asset filename, and resulting download URL are the version contract. Do not add or depend on a separate runtime marker/version file inside `mega-phgo-runtime`; a folder is acceptable only when the PHgo probe and runtime-owned MUSCLE check pass, and new runtime releases are picked up through the manifest URL/filename.
  - PHgo runtime downloads must run without an HTTP timeout, like BLAST+ downloads. The local executable probe is separate from downloading and may use a generous bounded timeout only to avoid a permanently hung bad runtime.
  - Protein FASTA selected for tree analysis may contain terminal stop codons from source exports. Preserve the original PHgo `input.fasta`, but the custom runtime must sanitize protein/unknown non-codon sequences before alignment: trim terminal `*`, convert internal `*` to `X`, convert `.`/`~` to gaps, and convert other unsupported protein characters to `X`. Record cleanup counts in `runtime.log` with `protein.sanitized`; do not apply this cleanup to nucleotide/codon methods.
  - Canvas tree conversion is a first-class tree setting. The default target is Protein mode, mismatch handling defaults to Convert, and skipped/failed-conversion rows are unchecked by default. Go may classify and orchestrate rows, but DNA-to-protein conversion must be done by the custom MEGA PHgo runtime using MEGA translation code; do not reintroduce local Go codon tables or Go-side DNA-to-protein translation.
  ...
  - Protein-to-DNA tree input is only valid when PHgo already has a real nucleotide sequence for that row, either embedded in the row or resolved through the current source cache/resolver. Do not invent reverse translations from protein FASTA. If a protein-only row is used in DNA mode, it must be skipped or reported explicitly.
  - Tree execution must be self-checking after the runtime returns. If the aligned FASTA count does not match the selected records, or if the returned aligned sequences do not match the requested biological target, PHgo must raise an explicit error rather than silently accepting the wrong tree artifacts.
  - Tree artifact reuse is allowed only after the reused `aligned.fasta` passes the same biological-target validation as a fresh runtime execution. Reopened snapshots may contain older artifacts produced before a runtime fix; refresh must invalidate those stale wrong-kind artifacts and rerun `mega-phgo-runtime` instead of pushing the old payload back to Reactree.
  - In DNA mode, FASTA rows that contain protein sequence may be converted only by resolving a real nucleotide/CDS sequence from row metadata. PHgo should use row-specific source metadata first, including source database, proteome/genome ID, Phytozome genome text, report/header URL, or recognizable PHGO FASTA headers such as `phgo://Arabidopsis thaliana TAIR10/...`. If no real nucleotide resolver can be identified, the row must be skipped or reported; never reverse-translate.
...
- Opening a Canvas snapshot must rebuild enough in-memory tree plan state from the saved payload, run manifest, and restored artifact files for the next tree-panel open, snapshot save, or preview-only metadata sync to continue without recomputing. If `last_payload` is empty but `viewer.payload.json` exists in artifacts, use the artifact payload as a recovery fallback. However, the first user-triggered `Refresh tree` after opening a Canvas snapshot is always a full compute refresh through `mega-phgo-runtime`; it must not reuse snapshot-restored alignment/tree artifacts even when fingerprints match.
  - Canvas tree refresh has two explicit phases: compute and render. The first refresh in a live Canvas session is full compute plus render. Later refreshes run render-only only when the sole change since the last successful compute is a display label change (`display_name` edits or the display-name source). Any other right-panel setting change, including conversion target/action, skip-and-unselect behavior, alignment method/parameters, or tree method/parameters, is a computation change and must rerun `mega-phgo-runtime`. Any Canvas table selection change is also a computation change.
  - Tree recomputation must be driven by computation inputs: selected row identity/order, sequence content, conversion target/action/skip-unselect settings, alignment method/parameters, and tree method/parameters. `display_name` edits and display-name source changes are preview-only and must update Reactree metadata without rerunning `mega-phgo-runtime` when the compute fingerprints are unchanged and the session is not in the first-refresh-after-snapshot state.
  - Tree alignment method surfaces must follow the conversion target. Protein mode shows the two amino-acid aligners; DNA mode shows the two base aligners plus the two codon aligners. Runtime plan building must reject incompatible stale/snapshot-selected methods before launching `mega-phgo-runtime`.
...
- The current unreleased `v2.2` contract should preserve only the explicitly recorded workflow artifacts and cache payloads that are needed for later continuity, not blanket `output/` or `.cache/` trees; snapshot open should restore only those recorded paths back to their original locations before later workflow actions continue. If a recorded file cannot be restored, the fallback must be explicit rather than silent.
...
- The bundled startup cleaner is also the release self-update entrypoint for packaged builds:
  ...
  - self-update must preserve only `output/`; application-local managed tool folders such as `blastplus/` and `mega-phgo-runtime/` must be replaced by the new bundle or recreated later on demand instead of being carried forward from the old install
...
- Release packaging rules:
  ...
  - `mega-phgo-runtime` release assets must be pre-extracted runtime directories in `assets/mega-phgo-runtime/<platform>/runtime/`, with release zips generated from those runtime directories only; each runtime directory must contain the PHgo runtime executable plus the runtime-owned MUSCLE binary (`muscleWin64.exe`, `muscleUnix64.exe`, or `muscledarwin64`); do not keep `.deb` or `.pkg` installers in the repository or use them at application runtime
  - the runtime release contract is source-controlled in `internal/megaphgo/runtime-release.json`: when the runtime changes, update that file's GitHub release tag plus exact published asset names during development, publish those assets to GitHub, and keep the app-side runtime download URL logic synchronized only through that manifest instead of deriving names from the app build version at release time. Do not add runtime marker files or marker-based freshness checks; the manifest release tag plus asset filename/download URL is the only runtime version coupling. Until non-Windows runtime builds are deliberately supported, this manifest should list only the Windows amd64 runtime package.
  - Windows `mega-phgo-runtime` is currently built and verified. Linux/macOS runtime packages are not built or published for current releases; the app must directly report managed PHgo runtime unsupported on Linux/macOS. If future Linux/macOS support is explicitly requested, those assets must be real PHgo runtime executables built on matching platforms or with a configured Lazarus/FPC cross toolchain; never use MUSCLE-only folders, MEGA-CC binaries, installed MEGA files, or renamed placeholders to satisfy release validation.
  - `.github/workflows/mega-phgo-runtime.yml` is a reserved future native multi-platform runtime build path. Do not use it to publish Linux/macOS runtime packages for current releases unless Linux/macOS PHgo runtime support has been intentionally re-enabled.
...
- Exported `.xlsx`, `.fasta`, and PDF data analysis reports are written under an `output/` directory next to the executable being run.
