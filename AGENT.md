...
- TAIR release support is intentionally maintained as an explicit per-release asset table in `internal/tair`; this is acceptable and preferred over guessing across TAIR's heterogeneous historical directory layouts. Verify paths against TAIR's JSON download listing endpoint (`https://www.arabidopsis.org/api/download-files/list?dir=...`) and download concrete files through `https://www.arabidopsis.org/api/download-files/download?filePath=...`. For versioned releases that expose GFF/FASTA assets, keyword rows, peptide export, nucleotide export, BLAST databases, and row enrichment must come only from that selected TAIR release's official download files. Do not fall back from a selected TAIR release to Ensembl, UniProt, Phytozome, or TAIR live search APIs for sequence data or version-specific row enrichment. TAIR live APIs may be used only for TAIR workflows with no versioned release assets available, and such releases should remain hidden from modes that require unavailable assets.
