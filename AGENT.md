...
- Symbol name identification is a standalone system under `internal/labelname`; keep the package/import name for compatibility, but user-facing text should say `symbol name`. Workflows send known structured fields from source rows, BLAST rows, query sources, and FASTA/phgo headers as `AliasRankRequest` data. The symbol name system owns matching and ranking.
  ...
  - Runtime install/update checks should prefer the GitHub-hosted prebuilt symbol-name database manifest and archive from the dedicated repository `KiriKirby/phytozome-go-symbolname-db`, not direct NCBI download/build. The prebuilt branch stores only `symbolname/manifest.json` and small documentation files; database archive files are distributed through that repository's GitHub Release assets and referenced by `browser_download_url` values in the manifest. Runtime downloads those assets, expands them locally to `symbolname.pgd`, and uses the manifest's NCBI split-source `Last-Modified` metadata for staleness checks. Only when the GitHub manifest/archive path is unavailable or invalid should the app fall back to direct NCBI `GENE_INFO/` split-source download and local build.
  ...
  - Local `.pgd` builds must optimize for sorted bbolt writes: parse/generate index terms concurrently, batch prepared records, sort record writes by id, sort index writes by `term/id`, then commit. Avoid per-index-entry read-before-write checks because `term/id` entries are already unique after term compaction.
  ...
  - Every `GENE_INFO/` download/build path, including startup helper and in-app TUI installation, must show visible progress with downloaded bytes, total size when known, download speed, and database build progress/record count. Runtime fallback transfers against NCBI should download one split file at a time instead of ranged multi-connection downloads against the FTP host; GitHub Actions prebuilds may use modest bounded parallel downloads for independent split files.
...
- Bundled WezTerm startup has one special packaging-only preflight: when the packaged app itself is opened, tab `[0]` runs `phgohelper.bin` with visible text/spinner feedback, then that helper should open the main wizard through `core.bin` in a fresh tab in the same WezTerm window unless the user chose "use while downloading" and it already launched the main tab. User-created new tabs inside that same WezTerm window must bypass cache cleaning and launch `core.bin` directly.
- The bundled startup helper is also the release self-update entrypoint for packaged builds:
  ...
  - after the release self-update check, the startup helper `phgohelper.bin` checks the GitHub-hosted prebuilt symbol name library manifest first, falling back to NCBI Gene `GENE_INFO/` split-source metadata only if GitHub is unavailable. If `symbolname.pgd` is missing, it shows the source, remote size, Last-Modified, and target path, then asks y/n whether to download/build now; declining skips only startup preflight, and the TUI must ask/download when symbol names are later used. If the remote source `Last-Modified` is newer than the stored value, it shows the expired day count and remote size before asking whether to update. If an application update was accepted and staged, skip the symbol library check until the next launch. Runtime NCBI fallback downloads split files sequentially rather than using ranged multi-connection downloading; build/index work may still use local concurrency where safe. Startup downloads and update downloads must show progress, speed, and build status.
  - packaged startup uses tab `[0]` for `phgohelper.bin` and `core.bin` for the main TUI. Until tab `[0]` finishes initialization, new `core.bin` tabs wait in a modal. If startup download is running with "use while downloading" enabled, new tabs may enter the app, but any symbol-name operation must wait for tab `[0]` to finish the database download/build before continuing.
...
- Cache layout rule:
  ...
  - packaged desktop startup uses a dedicated `[0]` helper tab that cleans only `.cache`, then checks GitHub releases for a newer packaged bundle, and finally spawns the main wizard through `core.bin` in a fresh tab in the same WezTerm window and exits; user-created tabs inside that same WezTerm session must bypass cache cleaning and launch the main wizard directly
